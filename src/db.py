    ...
    with conn:
        ...
        c.execute(
            """INSERT INTO game (gamemode, options, started, finished, gamesize, winner)
                     VALUES (?, ?, ?, ?, ?, ?)""",
            (mode, json.dumps(options), started, finished, size, winner),
        )
        gameid = c.lastrowid
        for p in players:
            c.execute(
                """INSERT INTO game_player (game, player, team_win, indiv_win, dced)
                         VALUES (?, ?, ?, ?, ?)""",
                (gameid, p["playerid"], p["won"], p["iwon"], p["dced"]),
            )
            gpid = c.lastrowid
            c.execute(
...
def get_player_stats(acc, hostmask, role):
    ...
    if row:
        return (
            "\u0002{0}\u0002 as \u0002{1[0]}\u0002 | Team wins: {1[1]} ({2:.0%}), "
            "Individual wins: {1[2]} ({3:.0%}), Overall wins: {1[3]} ({4:.0%}), Total games: {1[4]}."
        ).format(name, row, row[1] / row[4], row[2] / row[4], row[3] / row[4])
...
def get_player_totals(acc, hostmask):
    ...
    return "\u0002{0}\u0002's totals | \u0002{1}\u0002 games | {2}".format(
        name, total_games, break_long_message(totals, ", ")
    )
...
def _migrate():
    ...
    dn = os.path.dirname(__file__)
    conn = _conn()
    with conn, open(os.path.join(dn, "db", "db.sql"), "rt") as f1, open(
        os.path.join(dn, "db", "migrate.sql"), "rt"
    ) as f2:
        c = conn.cursor()
        #######################################################
...
def _get_display_name(peid):
    ...
    c = conn.cursor()
    c.execute(
        """SELECT COALESCE(pp.account, pp.hostmask)
                 FROM person pe
                 JOIN player pp
